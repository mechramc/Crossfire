...
- [ ] T-0609a Port CoreML-LLM `ChunkedEngine.swift` to Python for Mac harness — orchestrate token -> QuantEmbed int8 lookup -> PerLayerRawEmbed (PLE) lookup -> chunk1/2/3 forward passes with stateful KV + RoPE -> LM head argmax. Reference Python helpers already in `vendor/coreml-llm/conversion/collect_eagle_hidden_states_w4a8.py` (`QuantEmbed`, `PerLayerRawEmbed`, RoPE table loaders). Swift orchestration at `vendor/coreml-llm/Sources/CoreMLLLM/ChunkedEngine.swift`. Target: correct-output text generation at >= iPhone 17 Pro's 31 tok/s on M4 Max
