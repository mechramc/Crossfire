...
- [~] T-0615 Record baseline perplexity runs (wikitext-2-raw-v1, 20 chunks, c=512). Subtasks split by node below. Mac side DONE Session 26; PC side DONE Session 25. Cross-node investigation CLOSED Session 27: **the 4.6× PC↔Mac gap is 4× harness/binary + 1.15× corpus** (PC cross-corpus run on committed Mac file = 2,991 vs PC local-mirror = 2,595 vs Mac same file = 11,896). Corpus-level differences are minor; binary/fork differences dominate. Also CLOSED Session 27: **slot-bank ≠ stock numerics on Gemma 4 26B-A4B** — prefetch explains 25% of divergence, remaining 25% is in the slot-bank kernel itself. C7 canonical config will be slot-bank +prefetch with the PPL divergence documented as an open upstream question.
...
- [x] T-0615.mac Mac: three variants DONE Session 25. **31B Q8_0 PPL = 11,896 ± 1,234**, **26B-A4B fp16 stock PPL = 57,163 ± 6,273**, **26B-A4B fp16 slot-bank (16 slots, topk=8, prefetch-temporal) PPL = 38,957 ± 4,245**. Prefill rates 269 / 1,606 / 12 tok/s respectively. All three via `llama-perplexity` on M4 Max Metal; 26B-A4B variants via `vendor/anemll-flash-llama.cpp/` build. **Numerical finding flagged for C7**: slot-bank mode is NOT bit-identical to stock on Gemma 4 26B-A4B — chunk 1 matches within 0.5% but subsequent chunks diverge by up to 32%, likely from `--moe-prefetch-temporal` biasing routing across tokens. Re-run with `--no-moe-prefetch-temporal` queued as a numerics-isolation follow-up. Corpus: `datasets/wiki.test.raw` (Salesforce/wikitext wikitext-2-raw-v1 test split via HF). Summary: `results/t0615_t0616_mac.json`. Logs: `results/raw/t0615_mac_*_ppl.log` (gitignored).
...
- [!] T-0617 Record distributed P1 baseline at 8K / 16K / 32K (C1) — BLOCKED upstream Session 28. EXO 1.0 MlxRing tensor-parallel ring backend silently falls back to CPU on Linux/WSL2 CUDA peers (independent repro: dev.to — "MLX CUDA ring implementation simply doesn't work yet as of MLX 0.31.1"). Got topology + libp2p ring up (Windows firewall rule for ephemeral 49153-65535), got MlxRing init to report complete, applied skip-warmup patch to EXO (`src/exo/worker/engines/mlx/generator/generate.py` — bypasses JIT cascade when `EXO_SKIP_WARMUP=1` set on both ranks). LoadModel finished on Mac in 11.87s; on PC CPU ran >5 min and got kicked as inactive. First prefill ran 4+ min with 0 decode tokens, RTX 5090 at 8% util / 65W, runner at 363% CPU. Pipeline sharding originally rejected by EXO for Gemma 4 ("use tensor parallelism instead"). **Session 27 PC-side EXO branch `feat/gemma4-pipeline-port` lifts that rejection** (Tasks 1–6 of `docs/plans/2026-04-24-gemma4-exo-pipeline-port.md`; tuple-passthrough fix landed at EXO commit `94c0ce6` — `PipelineLastLayer.__call__` now passes `(h, shared_kv, offset)` from `Gemma4TextModel.DecoderLayer` through correctly; only `output[0]` crosses the rank boundary; bundle at `C:\Users\mechr\Downloads\gemma4-pipeline-fix-94c0ce6.bundle`). Pipeline LOAD now works (PC + Mac each shard 30 layers, ~46 s cold) but Pipeline INFERENCE hangs at `mx_barrier` (`generate.py:326`) on first `mx.distributed.all_sum` — same broken CUDA ring stub. Lifting the EXO-side ban does not (and cannot) fix the MLX-side CUDA ring. Options for the C1 row recorded in `results/t0617_p1_distributed_baseline.json`: (A) wait for MLX upstream fix, (B) use disaggregated prefill/decode mode when exposed, (C) pivot to llama.cpp RPC, (D) ship C0-C7 matrix without C1. Recommended: D for now, revisit when MLX/EXO land CUDA ring.
